...
sample_ca_hc <- scale(sample_maha_updated)
headTail(sample_ca_hc)
...
library(cluster)
ac <- function(x){
  agnes(sample_ca_hc, method =x)$ac
}
```

# Calculate agglomerative coeficient for each clustering linkage method
```{r}
sapply(m, ac)
...
library(factoextra)
gap_stat_h1 <- clusGap(sample_ca_hc, FUN = hcut, rstart = 25, K.max = 10, B = 50)
# gap_stat_k1 <- clusGap(sample_ca, FUN = kmeans, nstart = 25, K.max = 10, B = 50)
gap_stat_h2 <- clusGap(sample_ca_hc, FUN = hcut, rstart = 25, K.max = 20, B = 50)
...
distance_mat1 <- dist(sample_ca_hc, method = "euclidean")
...
final_ca <- cbind(sample_ca_hc, cluster = fit1)
head(final_ca)
hcentres1 <- aggregate(x=final_ca, by=list(cluster=fit1), FUN="mean")
print(hcentres1)

final_ca_1 <- cbind(sample_ca_hc, cluster = fit1_1)
head(final_ca_1)
...
```
From the plot we can see that the gap statistic is high at k = 4.

```{r}
fviz_gap_stat(gap_stat_k2) # without outliers
```
From the plot we can see that the gap statistic is high at k=3. This suggests that the outliers may form a cluster of their own.
...
```{r}
set.seed(55)
k_cl2 <- kmeans(sample_maha_updated,6,nstart=25) # without outliers
k_cl2
```
...
```{r}
sample_maha$cluster <- k_cl$cluster
sample_maha_updated$cluster <- k_cl2$cluster
```

# Create elbow plot
```{r}
sample_maha$cluster <- as.factor(sample_maha$cluster)
...
for (k in k_values) {
  wss_list[[k]] <- calculate_wss(features, k)
}
