...
#corrplot(correlation_matrix_spearman, tl.cex = 0.6)
#```


### Method 2 (same correlation data, different visualisation styles)(Remove method1)
```{r}
...
corrplot(correlation_matrix_spearman, method = "color")
corrplot(correlation_matrix_spearman, method = "color", addCoef.col = "black",
         tl.col = "black", tl.srt = 45, tl.cex = 0.6,number.cex = 0.4, mar = c(0,0,1,0),
         width = 30, height = 30)
```
...
```{r}
sample_pca <- sample_maha_updated[, c(1, 2, 3, 5, 11, 12, 13, 16 )] 
lowerCor(sample_pca)
```
...
```{r}
KMO(sample_pca)
#KMO > 0.5 then we conclude that some variables are highly correlated
```
### The Bartlett test
```{r}
...
final_ca_1 <- cbind(sample_maha_updated, cluster = fit1_1)
head(final_ca_1)
hcentres1_1 <- aggregate(x=final_ca_1, by=list(cluster=fit1_1), FUN="mean")
print(hcentres1_1)
```
...
Silhouette plot
```{r}
fviz_nbclust(sample_maha, kmeans, method = "silhouette")
```

Create elbow plot (with outliers)
```{r}
fviz_nbclust(sample_maha, kmeans, method = "wss")
```
...
```{r}
fviz_nbclust(sample_maha_updated, kmeans, method = "wss")
```
...
# k = 5
set.seed(55)
k_cl <- kmeans(sample_maha,5,nstart=25) # with outliers
k_cl 
```
...
# k = 5
set.seed(55)
k_cl2 <- kmeans(sample_maha_updated,5,nstart=25) # without outliers
k_cl2
```
...
# k = 3
set.seed(55)
k_cl3 <- kmeans(sample_maha_updated,3,nstart=25) # without outliers
k_cl3
```

# Show centroid values
```{r}
k_cl$centers # with outliers, k = 5
```

```{r}
k_cl2$centers # without outliers, k=5
```

```{r}
k_cl3$centers # without outliers, k=3
```

# Visualise clusters
```{r}
fviz_cluster(k_cl, data = sample_maha) # with outliers, k = 5
```

```{r}
fviz_cluster(k_cl2, data = sample_maha_updated) # without outliers, k=5
```

```{r}
fviz_cluster(k_cl3, data = sample_maha_updated) # without outliers, k=3
```

# Assign cluster labels to each data point
```{r}
sample_maha_labelled <- sample_maha
sample_maha_labelled$cluster <- k_cl$cluster

sample_maha_updated_labelled <- sample_maha_updated
sample_maha_updated_labelled$cluster <- k_cl2$cluster
```

# See cluster size
```{r}
sample_maha_updated_labelled$cluster <- as.factor(sample_maha_updated_labelled$cluster)
summary(sample_maha_updated_labelled)
```

## In order to decide between Hierarchical and Non-Hierarchical Clustering, k=5, we use internal validation to find the % of cases that are assigned to the same cluster. 
...
# Cutting tree by no. of clusters
fit2 <- cutree(Hierar_valid, k = 5 )
fit2

final_ca2 <- cbind(sample_validation, cluster = fit2)
head(final_ca)
...
```{r}
set.seed(55)
k_cl_valid <- kmeans(sample_validation,5,nstart=25)
k_cl_valid$centers
```
