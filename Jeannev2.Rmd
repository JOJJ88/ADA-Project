...
# without outliers, k=3

  
# without outliers, k=5
...
# Left join labelled sample data with other variables in original population
```{r}
# Hierarchical clustering:
# without outliers, k=3
...
# K-means clustering:
# with outliers, k=4
result_w <- merge(sample_maha2, df, by = c("funded_amnt", "int_rate", "emp_length", "annual_inc", "dti", "inq_last_6mths", "pub_rec", "revol_bal", "revol_util", "total_acc", "tot_coll_amt", "tot_cur_bal", "total_credit_rv"), all.x = TRUE)

n_distinct(result_w$id) # double check that each and every row is unique

write.csv(result_w, "sample with outliers kmeans k=4.csv",row.names = FALSE) # output as csv file for further analysis

# without outliers, k=4
result_wo <- merge(sample_maha_updated2, df, by = c("funded_amnt", "int_rate", "emp_length", "annual_inc", "dti", "inq_last_6mths", "pub_rec", "revol_bal", "revol_util", "total_acc", "tot_coll_amt", "tot_cur_bal", "total_credit_rv"), all.x = TRUE)

n_distinct(result_wo$id) # double check that each and every row is unique

write.csv(result_wo, "sample without outliers kmeans k=4.csv",row.names = FALSE) # output as csv file for further analysis 
```
...
##In order to decide between Hierarchical and Non-Hierarchical Clustering, k=4, we use internal validation to find the % of cases that are assigned to the same cluster. 

## Internal Validation for Sample without Outlier

Because the smallest cluster only has 24 data points, which is 4.43% of the sample size, hence we decide to randomly take 200 data points for the validation sample in order to sample each cluster sufficiently.
...
Non-Hierarchical Clustering for validation sample
```{r}
set.seed(55)
k_cl_valid <- kmeans(sample_validation,4,nstart=25)
k_cl_valid$centers
```
